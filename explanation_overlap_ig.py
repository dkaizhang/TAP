...
from src.explainer import get_input_gradients
...
def parse_args():
    ...
    parser.add_argument('--log', type=str, default='results/top_ig.csv', help='path to log file')
...
def main(args):
    ...
    for batch_idx, data in enumerate(tqdm(loader)):
        ...
        act = get_input_gradients(model.model, x, y, keep_grad=False, device=device)

        # print('need to match decoy pos size: ', decoy_positions.shape)
        act = act.sum(dim=1)
        # print('act shape after summing: ', act.shape)
        resized_act = act
        ...
        coverages.append(coverage.detach())
    ...
    coverages = torch.cat(coverages).mean().item()
    precisions = torch.cat(precisions).mean().item()
