...
from src.wrapper import TAPModelWrapper
...
def main(args):
    ...
    model = TAPModelWrapper(model_name=config['model_name'], 
                        ...
                        penal_loss=config['penal_loss'],
                        penal_lambda=config['penal_lambda'],
                        tap_lambda=config['tap_lambda'],
                        l2=config['l2'],
                        lr=config['lr'],
                        weights=weights,
                        device=device, 
                        task_loss=config['task_loss'],
                        mask_threshold_wr=config['mask_threshold_wr'],
                        manual_mask_region=config['manual_mask_region'],
                        labels=labels)
